...
class WickrAdmin {
  ...
  updateAdminList() {
    ...
    if (!this.dotenv) {
      ...
      const dotenv = JSON.parse(processes)
      const wlUsers = this.adminIDs.join(',')
      if (dotenv.tokens.ADMINISTRATORS.encrypted) {
        const wlUsersEncrypted = WickrIOAPI.cmdEncryptString(wlUsers)
        dotenv.tokens.ADMINISTRATORS.value = wlUsersEncrypted
      } else {
        dotenv.tokens.ADMINISTRATORS.value = wlUsers
      }

      try {
        const cp = execSync('cp .env .env_backup.json')
        const ps = fs.writeFileSync('./.env', JSON.stringify(dotenv, null, 2))
      } catch (err) {
        console.log(err)
      }
    }
      ...
      if (action === 'list') {
        ...
        if (values.length >= 1) {
          ...
          if (values.length >= 1) {
            ...
            let uMessage = WickrIOAPI.cmdSendRoomMessage(vGroupID, reply)
            ...
            uMessage = WickrIOAPI.cmdSend1to1Message(this.adminIDs, donereply)
          }
        } else {
      ...
      if (this.verifyAutomatic !== true) {
        if (command === '/verify') {
          ...
          if (action.startsWith('getlist')) {
            ...
            if (mode === 'all') {
              const getVerifList = WickrIOAPI.cmdGetVerificationList(mode)
            } else {
              const getVerifList = WickrIOAPI.cmdGetVerificationList()
            }
              ...
              for (let i = 0; i < verificationList.users.length; i++) {
                if (
                  ...
                  if (reply.length + userreply.length >= 10000) {
                    const uMessage = WickrIOAPI.cmdSendRoomMessage(
                      vGroupID,
                      reply
                    )
            ...
            if (mode === 'automatic' || mode === 'manual') {
              ...
              const uMessage = WickrIOAPI.cmdSendRoomMessage(vGroupID, response)
            } else {
              const reply =
